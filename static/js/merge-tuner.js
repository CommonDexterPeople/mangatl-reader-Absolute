...
export function openMergeTuner(pageIdx) {
  ...
  modal.innerHTML = `
    <div class="flow-modal-inner merge-tuner-inner">
      <div class="flow-modal-head">
        <strong>Bubble Merge Sensitivity — page ${pageIdx + 1}</strong>
      ...
      <div class="merge-tuner-controls">
        <label for="merge-tuner-slider">Sensitivity</label>
        <input id="merge-tuner-slider" type="range" min="0.1" max="1.5" step="0.05">
        <span id="merge-tuner-val" class="merge-slider-val"></span>
        <span id="merge-tuner-count" class="merge-tuner-count"></span>
      </div>
      <div class="merge-tuner-foot">
        <button class="btn-secondary" onclick="closeMergeTuner()">Cancel</button>
        <button class="btn-primary" onclick="applyMergeTuner()">Use this value</button>
      </div>
    </div>`;
